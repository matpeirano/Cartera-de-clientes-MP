...
function inferTipo(instrumento, ticker, esExterior = false) {
  ...
  // LECAPs / LETRAS
  if (inst.includes('LECAP') || inst.includes('LETRA DEL TESORO') || inst.includes('LETRA CAP'))
                                                                       return 'LECAPs';
  ...
  // FCI
  const FCI_T = ['BCMMA','BCRFA','ESTRA1A','ESTRA2A','ESTRA3A','INSTITUA','CPRIVADOA','SOJAA','BCMMUSA'];
  ...
  // ACCIONES ARGENTINAS
  const ACC_ARG = ['GGAL','PAMP','TGNO4','TGSU2','MOLI','CARC','YPF','YPFD','CRES',
...
function renderAnalisis(rows) {
  ...
  const rfConVto = rows.filter(r => RENTA_FIJA_TIPOS.includes(r.tipo)).map(r => {
    const fv = extractFechaVto(r.instrumento);
...
/* ══════════════════════════════════════════
   VENCIMIENTOS
══════════════════════════════════════════ */
function extractFechaVto(instrumento) {
  // Pattern V22/08/26 or V2208026 etc
  const m = instrumento.match(/V(\d{2})\/(\d{2})\/(\d{2,4})/);
  if (m) {
...
const RENTA_FIJA_TIPOS = ['Obligaciones Negociables','Bonos Soberanos USD','Bonos CER','Bonos CAP',
  'Bonos Nacion $','Bonos Provinciales','BOPREAL','LECAPs','Treasuries'];
...
function renderVencimientos(rows) {
  ...
  renderTable('tablaVencimientos', conFecha, r => {
  ...
  renderTable('tablaVencSinFecha', sinFecha, r => {
    const col = TYPE_COLORS[r.tipo] || '#64748b';
    return `<tr>
      <td>${esc(r.comitente.split(' ')[0])}</td>
      <td><strong>${esc(r.ticker)}</strong></td>
...
function renderMovimientosDetalle() {
  ...
  const comunes  = rowsCurr.filter(r => mapP.has(key(r))).map(r => ({
    ...r, tenAnt: mapP.get(key(r)).tenencia, diff: r.tenencia - mapP.get(key(r)).tenencia
  }));
  ...
  const movRow = r => {
    const col = TYPE_COLORS[r.tipo]||'#64748b';
    const diff = r.diff ?? 0;
    const pct  = r.tenAnt ? diff/r.tenAnt*100 : null;
    ...
    return `<tr>
      ...
      <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis" title="${esc(r.instrumento)}">${esc(r.instrumento)}</td>
      <td><span class="tipo-badge" style="background:${col}22;color:${col}">${esc(r.tipo)}</span></td>
      <td class="tr">${r.tenAnt !== undefined ? fmtUSD(r.tenAnt) : '--'}</td>
      <td class="tr"><strong>${fmtUSD(r.tenencia)}</strong></td>
      <td class="tr ${cls}">${diff>=0?'+':''}${fmtUSD(diff)}</td>
  ...
  const newRow = r => {
    const col = TYPE_COLORS[r.tipo]||'#64748b';
    return `<tr>
      <td>${esc(r.comitente.split(' ')[0])}</td><td><strong>${esc(r.ticker)}</strong></td>
  ...
  renderTable('tablaMovNuevas',   nuevas,    newRow);
  renderTable('tablaMovCerradas', cerradas,  newRow);
  renderTable('tablaMovCrecieron', crecieron, movRow);
  renderTable('tablaMovCayeron',  cayeron,   movRow);
}
