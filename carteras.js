...
const RENTA_FIJA_TIPOS = ['Obligaciones Negociables','Bonos Soberanos USD','Bonos CER','Bonos CAP',
  'Bonos Nacion $','Bonos Provinciales','BOPREAL','Treasuries'];
...
function agruparPorTicker(rows, modo) {
  // modo: null=solo tenencia, 'crecio'/'cayo'=con diff
  const m = {};
  rows.forEach(r => {
    const t = r.ticker;
    if (!m[t]) m[t] = { ticker:t, instrumento:r.instrumento, tipo:r.tipo, tenencia:0, tenAnt:0, diff:0, count:0 };
...
function renderMovimientosDetalle() {
  ...
  const movRowAgrup = r => {
    const col = TYPE_COLORS[r.tipo]||'#64748b';
    const cls  = (r.diff ?? 0) >= 0 ? 'pos' : 'neg';
    const pct  = (r.tenAnt && r.diff !== undefined) ? r.diff/r.tenAnt*100 : null;
    return `<tr>
      <td class="tc">${r.count}</td>
      <td><strong>${esc(r.ticker)}</strong></td>
