...
function inferTipo(instrumento, ticker) {
  ...
  if (tic === 'U$S' || tic === 'U$SCV7000' || inst.includes('DOLAR') || inst.includes('DOLARES')) return 'Dolares/Efectivo';
  ...
  if (inst.includes('CLASE A') || inst.includes('CLASE B') ||
      ...
      inst.includes('PRIVADO') || inst.includes('SOJA'))             return 'FCI';
  const tieneVto = /V\d{2}\/\d{2}\/\d{2}/.test(inst) ||
                   inst.includes('STEP UP') || inst.includes(' ON ') ||
                   inst.includes('CL.') || inst.includes('OBLIGACION');
  if (tieneVto)                                                       return 'Obligaciones Negociables';
  if (inst.includes('BONO') || tic.startsWith('GD') || tic.startsWith('AL') ||
      tic.startsWith('AE')  || tic.startsWith('TTD') || tic.startsWith('TX') ||
      tic.startsWith('TC')  || inst.includes('REP. ARG'))            return 'Bonos Soberanos';
