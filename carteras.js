...
function inferTipo(instrumento, ticker, esExterior = false, comitente = '') {
  ...
  // ── ACCIONES ARGENTINAS ─────────────────────────────────────────────────
  // Palabras clave en instrumento que indican accion arg (local o custodiada exterior)
  // IMPORTANTE: no incluir "BONO" para no mezclar con bonos
  const esAccionArgInst =
    inst.includes('ESCRIT') || inst.includes('ESCRITURALES') ||
    inst.includes('1 VOTO') || inst.includes('ORDS."B"') ||
    inst.includes('ORDS."A"') || inst.includes('ORD."B"') ||
    inst.includes('ORD."A"') || inst.includes('ORDINARIAS') ||
    inst.includes('S.A. ORDS') || inst.includes('S.A.I.C') ||
    inst.includes('SOCIEDAD ANONIMA');

  const ACC_ARG = ['GGAL','PAMP','TGNO4','TGNO','TGSU2','TGSU','MOLI','MOLA','CARC',
                   'YPF','YPFD','CRES','COME','IRSA','ECOG','ALUA','TXAR','BBAR','BMA',
                   'SUPV','MIRG','BYMA','HARG','LOMA','DGCU2','CEPU','GBAN','TECO2','VALO',
                   'TRAN','EDN','LEDE','RICH','SEMI'];
  if (ACC_ARG.includes(tic) || esAccionArgInst)                       return 'Acciones Arg.';
