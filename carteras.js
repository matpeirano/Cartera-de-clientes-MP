...
function populateFilters(rows) {
  // Build map: comitente number -> display label (account name max 3 words + number)
  const comiMap = {};
  rows.forEach(r => {
    if (r.comitente && !comiMap[r.comitente]) {
      const words = (r.cuenta || '').trim().split(/\s+/).slice(0, 3).join(' ');
      comiMap[r.comitente] = words ? `${words} (${r.comitente})` : r.comitente;
    }
  });
  const comis = Object.keys(comiMap).sort((a,b) => {
    // Sort by display name alphabetically
    return comiMap[a].localeCompare(comiMap[b], 'es');
  });
  ...
  selC.innerHTML = '<option value="TODOS">Todos los comitentes</option>' +
    comis.map(c => `<option value="${esc(c)}">${esc(comiMap[c])}</option>`).join('');
...
function getFiltered() {
  ...
  return STATE.actual.filter(r =>
    ...
    (!q || r.ticker.toUpperCase().includes(q) ||
          r.instrumento.toUpperCase().includes(q) ||
          (r.cuenta||'').toUpperCase().includes(q) ||
          r.comitente.toUpperCase().includes(q))
  );
}
...
function renderResumen(rows) {
  ...
  // Agrupar por ticker y sumar tenencia de todos los clientes
  const byTicker = {};
  rows.forEach(r => {
    if (!byTicker[r.ticker]) byTicker[r.ticker] = { ticker: r.ticker, instrumento: r.instrumento, tipo: r.tipo, tenencia: 0 };
    byTicker[r.ticker].tenencia += r.tenencia;
  });
  const topPos = Object.values(byTicker).sort((a,b)=>b.tenencia-a.tenencia).slice(0,8);
